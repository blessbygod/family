define("/javascripts/main", [], function(require) {
 ...
 * 2. 目前支持给定容器填充
 * */
define("/javascripts/plugins/jquery.pagination", [], function(require) {
    $.fn.pagination = function(options) {
        var $container = $(this);
        var _container = this;
        //初始化参数
        var template = [ '<ul class="<%= paginationULClass %>">', '<li class="<%= preAllClass %>">', '<a href="javascript:;"><%= _.isArray(paginationTextArr) ? paginationTextArr[0] : "" %></a>', "</li>", '<li class="<%= preOneClass %>">', '<a href="javascript:;"><%= _.isArray(paginationTextArr) ? paginationTextArr[1] : "" %></a>', "</li>", '<li class="<%= paginationSelectContainerClass %>">', '<select class="pagination-select span1" value="<%= current %>">', "<% for(var i=1; i <= pages; i++){ %>", '<option value="<%= i %>"><%= i %></option>', "<% } %>", "</select>", "</li>", '<li class="<%= nextOneClass %>">', '<a href="javascript:;"><%= _.isArray(paginationTextArr) ? paginationTextArr[2] : "" %></a>', "</li>", '<li class="<%= nextAllClass %>">', '<a href="javascript:;"><%= _.isArray(paginationTextArr) ? paginationTextArr[3] : "" %></a>', "</li>", "</ul>" ].join("");
        //初始化配置
        ...
        this.bindEvents = function() {
            ...
            var clickClasses = [ "." + options.preOneClass, "." + options.preAllClass, "." + options.nextOneClass, "." + options.nextAllClass ].join();
 ...
 * css 部分依赖bootstrap
 * */
define("/javascripts/plugins/jquery.togglelist", [], function(require) {
...
(function(factory) {
    if (typeof define === "function") {
        define("thirdparty/backbone.js", [], factory);
